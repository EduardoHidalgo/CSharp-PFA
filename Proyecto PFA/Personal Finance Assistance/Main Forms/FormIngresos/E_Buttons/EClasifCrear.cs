...
{
    public partial class EClasifCrear : UserControl
    {
        FormIngresos Ingresos;

        public EClasifCrear(FormIngresos ingresos)
        {
            Ingresos = ingresos;
            InitializeComponent();
        ...
        public void ButtonMethod()
        {
            ...
            if ((P1 & P2) == true)
            {
                try
                {
                    ...
                    DBclass.Insert_Tipo_E(Tipo, Fecha, Alg); //Crea la nueva clasificación. Actualiza la DB
                    TextboxClasif.Clear();
                    ComboboxClasif.ResetText();
                    LabelError.Text = "Clasificación de Egresos creada";
                    LabelError.Visible = true;
